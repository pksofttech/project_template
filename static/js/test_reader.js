...
let card_no = "1234";
window.test_reader = test_reader;
async function test_reader(device_name) {
    unity.logger.debug("test_reader");
    const input_card_no_html = `<label class="input input-bordered flex items-center gap-2">
                                <i class="fa-solid fa-card"></i>
                                <input type="text" class="grow" placeholder="card id" value='${card_no}' data-field="returnValue" />
                                </label>`;
    const result = await unity.showDialogConfirm({
        title: "Enter Card ID for Reader Test",
    ...
    if (result.confirm) {
        if (result.value != "") {
            ...
            const _result = await unity.fetchApi(
                `/api/devices/reader_device/reader?reader_name=${device_name}&card_id=${card_no}&card_type=VISITOR&test_mode=true`,
                "post",
                null,
                "json"
            );
...
async function gen_test() {
    const test_reader_content = document.getElementById("test_reader_content");
    if (test_reader_content) {
        const _reply = await unity.fetchApi("/api/devices/reader_device", "get", null, "json");

        let script_for_test = "";
        if (_reply && _reply.success && _reply.data && _reply.data.length > 0) {
            const readers = _reply.data;
            for (const reader of readers) {
                unity.logger.debug(reader);

                let badgeClass = "badge-info";
                let iconClass = "fa-right-to-bracket text-info";
                let borderHover = "hover:border-info/40 hover:shadow-info/10";

                if (reader.gateway_type === "IN") {
                    badgeClass = "badge-info";
                    iconClass = "fa-right-to-bracket text-info";
                    borderHover = "hover:border-info/40 hover:shadow-info/10";
                } else if (reader.gateway_type === "OUT") {
                ...
                script_for_test += `
                <button type="button" 
                        onclick="test_reader('${reader.device_name}');" 
                        title="${reader.device_name} - ${reader.gateway_name || ''} (ID: ${reader.id})"
                    ...
                    <div class="flex items-center justify-between gap-1 mb-1">
                        <div class="flex items-center gap-1.5 min-w-0">
                            ...
                            <span class="font-bold text-xs text-base-content group-hover:text-primary transition-colors truncate">${reader.device_name}</span>
                        </div>
                        <span class="badge ${badgeClass} badge-xs font-extrabold uppercase shrink-0 text-[9px] px-1">${reader.gateway_type || 'READER'}</span>
                    </div>
                    
                    <div class="flex items-center justify-between text-[10px] text-base-content/60 gap-1 pt-1 border-t border-base-content/5">
                        <span class="truncate font-medium">${reader.gateway_name || ''}</span>
                        <span class="font-mono bg-base-200/90 px-1 py-0.2 rounded-box text-[9px] font-semibold shrink-0">#${reader.id}</span>
                    </div>
                </button>`;
            }
        } else {
            ...
            <div class="col-span-full p-4 rounded-box bg-base-100/50 border border-base-content/10 text-center flex flex-col items-center justify-center gap-1 text-base-content/50 text-xs">
                <i class="fa-solid fa-microchip text-2xl opacity-40 mb-0.5"></i>
                <p class="font-bold">No Gate Reader configured in system</p>
            </div>`;
        }
        test_reader_content.innerHTML = script_for_test;
    }
}
gen_test();
